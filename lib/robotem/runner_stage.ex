defmodule Robotem.Runner.SingleStage do
  @moduledoc """
  ...
  def start_link(args) do
    ...
    GenStage.start_link(__MODULE__, args, name: args.process_module)
  end
  ...
  def handle_events(events, _from, state) do
    IO.puts("received events")

    Enum.each(events, fn {event, meta} ->
      handle_new_event(event, meta, state)
    end)
  ...
  defp handle_new_event(event, meta, state) do
    Task.start(fn ->
      ProcessConfiguration.update_last_seen(state.process_module, meta.seq)
    end)
    ...
    Logger.info("Task started for #{event.event_type}, ref = #{inspect(ref)}")
  ...
  defp handle_task_failure(ref, state) do
    event_id = Map.get(state.refs, ref)
    ...
    case task_data do
      %{attempt: attempt, event: event, meta: meta} when attempt < state.max_attempts ->
        delay = trunc(:math.pow(2, attempt) * @retry_delay_base)
  ...
  defp process_event(event, meta, module) do
    module.handle_envelope(event, meta)
  end
end
